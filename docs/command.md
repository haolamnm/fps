...
```bash
python -m fps.tools.index PHASES --id VIDEO_IDs
```

For example, to create index for `clip-openai` and `clip-laion` features for video `001`:

```bash
python -m fps.tools.index clip-openai clip-laion --id 001
```
